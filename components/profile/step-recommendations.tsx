...
import type { NutritionRecommendations } from "@/app/onboarding/page"

interface StepRecommendationsProps {
  recommendations: NutritionRecommendations
  onBack: () => void
  onComplete: (finalRecommendations: NutritionRecommendations) => void
}

export function StepRecommendations({ recommendations, onBack, onComplete }: StepRecommendationsProps) {
  const [customValues, setCustomValues] = useState<NutritionRecommendations>(recommendations)
  const [isEditing, setIsEditing] = useState(false)

  const handleReset = () => {
    setCustomValues(recommendations)
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4">
        {macroItems.map((item) => {
          ...
          const value = customValues[item.key as keyof NutritionRecommendations]
          const originalValue = recommendations[item.key as keyof NutritionRecommendations]
