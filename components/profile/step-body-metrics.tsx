...
export function StepBodyMetrics({data, onUpdate, onNext, onBack}: StepBodyMetricsProps) {
    const isValid = data.height > 0 && data.weight_value > 0

    const handleWeightUnitChange = (unit: "kg" | "lb") => {
        if (unit === data.weight_unit) return

        const newWeight = unit === "lb" ? Math.round(data.weight_value * 2.20462) : Math.round(data.weight_value / 2.20462)
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                    ...
                    <span
                        className="text-sm text-emerald-600 font-semibold">{activityLabels[data.activity_degree]}</span>
                </div>
                <Slider
